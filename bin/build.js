...
// Ensure that multiple subprocesses are throttled per number of CPUs.
// Otherwise the subprocesses might overload the system.
var concurrentTasks = [];
var numCores = os.cpus().length;
for (var i = 0; i < numCores; i++) {
  concurrentTasks.push(Promise.resolve());
}
var taskIdx = -1;

function schedule(promiseFactory) {
  taskIdx++;
  if (taskIdx === numCores) {
    taskIdx = 0;
  }
  concurrentTasks[taskIdx] = concurrentTasks[taskIdx].then(promiseFactory);
  return concurrentTasks[taskIdx];
}
...
// uglify in a separate process for faster build times
function uglify(code, prepend, fileOut) {
...
// browserify in a separate process for faster build times
function doBrowserify(path, browserifyArgs) {
  return schedule(function () {
    var binPath = require.resolve('browserify/bin/cmd');
    var args = [binPath, '-p', 'bundle-collapser/plugin', '-t', 'es3ify'];
    args = args.concat(browserifyArgs);
    args.push(path);

    var child = spawn(process.execPath, args, {stdio: 'pipe'});
    var stream = duplexify(child.stdin, child.stdout);
    return streamToPromise(stream).then(function (bundle) {
      var code = bundle.toString('utf-8');
      code = derequire(code);
      return code;
    });
  });
...
// run rollup in a separate process for faster build times
function rollup(entry, fileOut) {
  return schedule(function () {
    var binPath = require.resolve('rollup/bin/rollup');
    var args = [binPath, '--format=cjs',
      '--external=' + external.join(','), entry];
...
function buildForNode() {
  return rollup('src/index.js', 'lib/index.js');
}
...
function buildForBrowserify() {
  return ncp('src', 'src_browser').then(function () {
    ...
    return rollup('src_browser/index.js', 'lib/index-browser.js');
  });
}

// build for the browser (dist)
function buildForBrowser() {
  return doBrowserify('.', '-s PouchDB').then(function (code) {
    code = comments.pouchdb + code;
    return Promise.all([
      writeFile('dist/pouchdb.js', code),
      uglify(code, comments.pouchdb, 'dist/pouchdb.min.js')
    ]);
  });
}

function buildPerformanceBundle() {
  var args = ['--debug', '--full-paths'];
  return doBrowserify('tests/performance', args).then(function (code) {
...
function buildPluginsForBrowserify() {
  return Promise.all(plugins.map(function (plugin) {
    return rollup('src_browser/plugins/' + plugin + '/index.js',
...
function buildExtras() {
  return Promise.all(Object.keys(extras).map(function (entry) {
    ...
    return rollup('src_browser/' + entry, 'lib/extras/' + target);
  }));
}
...
function buildPluginsForBrowser() {
  return Promise.all(plugins.map(function (plugin) {
    ...
    return doBrowserify(source, ['-x pouchdb']).then(function (code) {
      code = comments[plugin] + code;
      return Promise.all([
        writeFile('dist/pouchdb.' + plugin + '.js', code),
        uglify(code, comments[plugin], 'dist/pouchdb.' + plugin + '.min.js')
      ]);
    });
  }));
}
...
function buildPlugins() {
  return buildPluginsForBrowserify().then(buildPluginsForBrowser);
}

console.log('Building with ' + numCores + ' concurrent cores...');

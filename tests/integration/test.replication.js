...
adapters.forEach(function (adapters) {
  describe('test.replication.js-' + adapters[0] + '-' + adapters[1],
    ...
    // Forbidden errors created from validate_doc_update should have the message
    // defined in PourchDB.Errors.FORBIDDEN instead of the thrown value.
    // #3070 sets error.reason to the thrown forbidden value.
    it('#3171 forbidden message should match errors.FORBIDDEN',
        function (done) {
      testUtils.isCouchDB(function (isCouchDB) {
        if (adapters[1] !== 'http' || !isCouchDB) {
          return done();
        ...
        return remote.put(ddoc).then(function () {
          ...
          res.errors[0].message.should.equal(forbiddenErrorMessage);
          res.errors[1].message.should.equal(forbiddenErrorMessage);

...
describe('revs diff', function () {
  adapters.map(function (adapter) {
    describe(adapter, function () {
      ...
      it('Missing docs should be returned with all revisions being asked for', function (done) {
        testUtils.initTestDB(testHelpers.name, function (err, db) {
          // empty database
          var revs = [
              '1-a',
      ...
      it('Conflicting revisions that are available should not be marked as' + ' missing (#939)', function (done) {
        var doc = {
            _id: '939',
            _rev: '1-a'
          };
      ...
      it('Deleted revisions that are available should not be marked as' + ' missing (#935)', function (done) {

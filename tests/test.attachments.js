...
QUnit.module("Attachments: " + db1 + " -> " + db2, {
  ...
  new PouchDB(db1, function(err, db) {
    db.bulkDocs({docs: docs}, function(err, res) {
      db.allDocs({include_docs: true}, function(err, res){
        for(var i = 0; i < 3; i++){
          for(var j = 0; j < i; j++){
            strictEqual(res.rows[i].doc._attachments['att' + j].stub, true,
                        '(allDocs) doc'+i+' contains att'+j+' stub');
          }
        }
        strictEqual(res.rows[0].doc._attachments, undefined,
        ...
        db.changes({
          ...
          onChange: function(change) {
            ...
            if (i === 0) {
              ...
              for(var j = 0; j < i; j++){
                strictEqual(res.rows[i].doc._attachments['att' + j].stub, true,
                            '(onChange) doc'+i+' contains att'+j+' stub');
              }
            }
          },
          complete: function(err, res) {
            ...
            for(var i = 0; i < 3; i++){
              for(var j = 0; j < i; j++){
                strictEqual(res.results[i].doc._attachments['att' + j].stub, true,
                            '(complete) doc'+i+' contains att'+j+' stub');
              }
            }
            strictEqual(res.results[0].doc._attachments, undefined,
...
asyncTest('Test get with attachments: true if empty attachments', function() {
  new PouchDB(db1, function(erro, db) {
          ...
          db.removeAttachment('mydoc', 'mytext1', rev, function(err, res) {
            db.get('mydoc', {attachments: true}, function(err, res) {
              ...
              db.removeAttachment('mydoc', 'mytext2', res._rev, function(err, res) {
                ok(res._attachments === undefined || !('mytext1' in res._attachments),
                   'attachment 1 correctly removed');
                ok(res._attachments === undefined || !('mytext2' in res._attachments),
                   'attachment 2 correctly removed');
                start();
              });
            });
          });
...
asyncTest("Attachments replicate", function() {
  ...
  var db = new PouchDB(db2, function() {

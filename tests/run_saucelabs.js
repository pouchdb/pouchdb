...
var RESULTS_HOST = 'http://reupholster.iriscouch.com/';
var LOCAL_RESULTS = 'test_suite_db110';
...
function replicate_test_results(sauce_details, couch, callback) {
  var db = couch.db.use(LOCAL_RESULTS);
  db.list({include_docs: true}, function(err, res){
    ...
    db.insert(doc, doc._id, function(err){
      if (err) {
        return callback('could not store test results');
      }
      var remote = RESULTS_HOST + 'pouch_tests';
      couch.db.replicate(LOCAL_RESULTS, remote, function(err) {
        callback(err, doc);
      });
...
function setTestPassed(browser, pass, callback) {
  var results = 'sauce:job-info={"passed": ' + pass + ', "public" : true  }';
  browser.setContext(results, callback);
}
...
browser
  ...
  .end(function(err) {
    ...
    replicate_test_results(sauce_details, couch, function(err, doc) {
      ...
      if (doc && doc._id) {
        console.log('Test details can be found at :\t' + RESULTS_HOST +
                    '/_utils/document.html?pouch_tests/' + doc._id);
      }

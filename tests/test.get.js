  ...
  new PouchDB(db1, function(err, db) {
    testUtils.writeDocs(db, JSON.parse(JSON.stringify(origDocs)), function() {
      db.get("3", function(err, parent){
        ...
        var conflicts = [
          {_id: "3", _rev: "2-aaa", value: "x",
           _revisions: {start: 2, ids: ["aaa", pRevId]}},
          {_id: "3", _rev: "3-bbb", value: "y", _deleted: true,
           _revisions: {start: 3, ids: ["bbb", "some", pRevId]}},
          {_id: "3", _rev: "4-ccc", value: "z",
           _revisions: {start: 4, ids: ["ccc", "even", "more", pRevId]}}
        ];

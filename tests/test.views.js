...
asyncTest("Test basic view", function() {
  new PouchDB(db1, function(err, db) {
    var docs = [{foo: 'bar'}, { _id: 'volatile', foo: 'baz' }];
...
asyncTest("Test opts.startkey/opts.endkey", function() {
  new PouchDB(db1, function(err, db) {
    var docs = [{key: 'key1'},{key: 'key2'},{key: 'key3'},{key: 'key4'},{key: 'key5'}];
    db.bulkDocs({docs: docs}, {}, function() {
      var queryFun = {
...
asyncTest("Test opts.key", function() {
  new PouchDB(db1, function(err, db) {
    var docs = [{key: 'key1'},{key: 'key2'},{key: 'key3'},{key: 'key3'}];
    db.bulkDocs({docs: docs}, {}, function() {
      var queryFun = {
        map: function(doc) { emit(doc.key, doc); }
      };
...
asyncTest("Test joins", function() {
  new PouchDB(db1, function(err, db) {
    var docs = [{_id: 'mydoc', foo: 'bar'}, { doc_id: 'mydoc' }];
    db.bulkDocs({docs: docs}, {}, function() {
      var queryFun = {
        map: function(doc) {
          if (doc.doc_id) {
...
asyncTest('Views should include _conflicts', function() {
  ...
  var db = new PouchDB(db1, function() {

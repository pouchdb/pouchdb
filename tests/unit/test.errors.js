...
describe('test.errors.js', function () {
  ...
  describe('PouchError', function () {
    it('should create an error with correct properties', function () {
      const error = new PouchError(404, 'not_found', 'missing');
  ...
  describe('createError', function () {
    it('should create a new error with inherited properties', function () {
      const baseError = new PouchError(400, 'bad_request', 'Something went wrong');
      baseError.customProp = 'custom';
      const newError = createError(baseError, 'new reason');
      newError.status.should.equal(400);
      newError.name.should.equal('bad_request');
      newError.message.should.equal('Something went wrong');
      newError.customProp.should.equal('custom');
      newError.reason.should.equal('new reason');
    });

    it('should return a proper string representation', function () {
      const baseError = new PouchError(400, 'bad_request', 'Something went wrong');
      baseError.customProp = 'custom';
      const error = createError(baseError, 'new reason');
      const errorString = error.toString();
      errorString.should.equal(
        JSON.stringify({
          status: 400,

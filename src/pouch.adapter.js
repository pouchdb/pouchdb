        ...
        if (opts.filter === '_view') {
          if (opts.view && typeof opts.view === 'string') {
            ...
            api.get('_design/' + viewName[0], function (err, ddoc) {
              if (ddoc && ddoc.views && ddoc.views[viewName[1]]) {
                ...
                err = err || 
                     (ddoc.views ? 
                      PouchUtils.error(Pouch.Errors.MISSING_DOC, 'missing json key: ' + viewName[1]) :
                      PouchUtils.error(Pouch.Errors.MISSING_DOC, 'missing json key: views'));
          ...
          api.get('_design/' + filterName[0], function (err, ddoc) {
            if (ddoc && ddoc.filters && ddoc.filters[filterName[1]]) {
              ...
              err = err || 
                   (ddoc.filters ? 
                    PouchUtils.error(Pouch.Errors.MISSING_DOC, 'missing json key: ' + filterName[1]) :
                    PouchUtils.error(Pouch.Errors.MISSING_DOC, 'missing json key: filters'));
              PouchUtils.call(opts.complete, err);
            }
          });
        }

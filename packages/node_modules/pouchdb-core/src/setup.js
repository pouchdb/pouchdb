...
function setUpEventEmitter(Pouch) {
  ...
  Pouch.on('unref', function onConstructorUnref(db) {
    ...
    var db_list = destructListeners.get(db.name);
    var pos = db_list.indexOf(db);
    if (pos < 0) {
      return;
    }
    db_list.splice(pos, 1);
    if (db_list.length > 1) {
      destructListeners.set(db.name, db_list);
    } else {
      destructListeners.delete(db.name);
    }
  });

  Pouch.on('destroyed', function onConstructorDestroyed(db) {
    if (!destructListeners.has(db.name)) {
      return;
    }
    var db_list = destructListeners.delete(db.name);

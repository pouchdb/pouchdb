...
function prepareForDestruction(self) {
  ...
  function onClosed() {
    ...
    // Remove our onConstructorDestroyed from the destruction listeners.
    if (!destructionListeners.has(self.name)) {
      return;
    }
    var current_set = destructionListeners.get(self.name);
    var pos = current_set.indexOf(onConstructorDestroyed);
    if (pos === -1) {
      return;
    }
    if (current_set.length === 1) {
      ...
      current_set.splice(pos,1);
      destructionListeners.set(self.name, current_set);
    }
  }

 ...
 *   indexer is invalid.
 */
export default function createAbstractMapReduce(localDocName, mapper, reducer, ddocValidator) {
  ...
  async function updatePurgeSeq(view) {
    ...
    try {
      const purges = await view.sourceDB.get('_local/purges');
      const purgeSeq = purges.purgeSeq;
      const res = await view.db.get('_local/purgeSeq').catch(defaultsTo({}));
  ...
  async function saveKeyValues(view, docIdsToChangesAndEmits, seq) {
    ...
    const lastSeqDoc = await view.db.get(seqDocId)
      .catch(defaultsTo({ _id: seqDocId, seq: 0 }));

    const docIds = mapToKeysArray(docIdsToChangesAndEmits);
    const listOfDocsToPersist = await Promise.all(docIds.map(function (docId) {
  ...
  async function updateViewInQueue(view, opts) {
    ...
    async function getRecentPurges() {
      const purgeSeq = await view.db.get('_local/purgeSeq')
        .then(function (res) {
          return res.purgeSeq;
        })
        .catch(defaultsTo(-1));
      ...
      try {
        ...
        return Promise.all(uniquePurges.map(function (docId) {
          return view.sourceDB.get(docId)
            .then(function (doc) {
              return { docId, doc };
            })

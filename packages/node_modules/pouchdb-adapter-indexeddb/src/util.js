...
//
// TODO: delete this, replace all calls with api._openTransactionSafely
//
// This function no longer makes sense in a world where your idb handle can be
// invalidated by someone needing to close and reopen the DB to re-generate
// indexes.
//
// Allowing the api to manage opening transactions means that you'll always use
// a fresh DB handle to open a transaction, which will remain valid until you
// close out the transaction.
//
function openTransactionSafely(idb, stores, mode) {
  console.warn('!!!!!!DEPRECATED OPEN TRANSACTION CALL!!!!');
...
function correctIndexFields(fields) {
  ...
  var correctedIndexFields = ['deleted'];

  fields.forEach(function (field) {
    if (field in ['_id', '_rev', '_deleted', '_attachments']) {
      // TODO: check that I'm dealing with these generated properties correctly
      correctedIndexFields.push(field.substr(1));
    } else {
      // The custom document fields are inside the `data` property
      correctedIndexFields.push('data.' + sanitise(field));
    }
  });

  return correctedIndexFields;
}

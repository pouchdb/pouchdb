...
function processAttachment(name, src, doc, isBinary, attachmentFormat) {
  ...
  console.log('processAttachment()', JSON.stringify({
    isBinary: String(isBinary),
    attachmentFormat: String(attachmentFormat),
    doc,
  }, null, 2));
  ...
  if (attachmentFormat === 'base64') {
    if (isBinary) {
      ...
      console.log('processAttachment()', ' pre:', src.attachments[doc._attachments[name].digest].data);
      doc._attachments[name].data = b64StringToBluffer(att.data, att.content_type);
      console.log('processAttachment()', 'post:', doc._attachments[name].data);
    } else {

...
function maintainNativeIndexes(openReq) {
  ...
  ddocsReq.onsuccess = function (e) {
    ...
    existingIndexNames.forEach(index => {
    ...
    var newIndexNames = expectedIndexNames.filter(ei => existingIndexNames.indexOf(ei) === -1);
    newIndexNames.forEach(indexName => {
...
function openDatabase(openDatabases, api, opts, resolve) {
...
export default function (openDatabases, api, opts) {
  if (!openDatabases[opts.name] || openDatabases[opts.name].versionchanged) {
    ...
    openDatabases[opts.name] = new Promise(function (resolve) {
      openDatabase(openDatabases, api, opts, resolve);
    });
  }

...
function openDatabase(openDatabases, api, opts, resolve, reject) {
  ...
  openReq.onsuccess = function (e) {
    ...
    // Always check blob support in case user's browser has changed since
    // original metadata creation.
    checkBlobSupport(txn, META_STORE, 'blob-support').then(blobSupport => {
      var metaStore = txn.objectStore(META_STORE);
      metaStore.get(META_STORE).onsuccess = function (e) {
        metadata = e.target.result || metadata;
        var changed = false;

        if (!('blob_support' in metadata) || metadata.blob_support !== blobSupport) {
          changed = true;
          metadata.blob_support = blobSupport;
        }

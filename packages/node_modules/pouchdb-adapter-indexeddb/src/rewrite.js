...
function rewriteValue(v) {
  if (v === null) return IDB_NULL;
  if (typeof v === 'boolean') return v ? IDB_TRUE : IDB_FALSE;
  if (typeof v === 'object') return rewriteObject(v);
  return v;
}
...
function rewriteObject(data) {
  ...
  if(Array.isArray(data)) return data.map(rewriteValue);

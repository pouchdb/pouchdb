...
function generateKeyRange(opts) {
  // TODO: is there a better way / existing way of doing this?
  function defined(obj, k) {
    return obj[k] !== void 0;
  }
  function fix(key, exact) {
    var fixed = [0].concat(key);

    // REVIEWER: Read this block carefully.
    //
    // This code is replacing *all* instances both the upper and lower collate
    // mark, regardless of where they are in the array.
    //
    // I am not sure if this is right.
    if (!exact) {
      fixed = fixed.map(function (k) {
  ...
  try {
    if (defined(opts, 'key')) {
      return IDBKeyRange.only(fix(opts.key, true));
    }

    if (defined(opts, 'startkey') && !defined(opts, 'endkey')) {
      return IDBKeyRange.lowerBound(fix(opts.startkey), !opts.inclusive_start);
    }

    if (!defined(opts, 'startkey') && defined(opts, 'endkey')) {
      return IDBKeyRange.upperBound(fix(opts.endkey), !opts.inclusive_end);
    }

    if (defined(opts, 'startkey') && defined(opts, 'endkey')) {
      return IDBKeyRange.bound(
        fix(opts.startkey),    fix(opts.endkey),
        !opts.inclusive_start, !opts.inclusive_end
      );
    }
...
function getIndexHandle(pdb, idb, fields, reject) {
  ...
  if (existingIndexNames.indexOf(indexName) === -1) {
    // The DB needs to be re-opened
    return pdb._freshen().then(function (idb) {
      return getIndexHandle(pdb, idb, fields, reject);
    });
  } else {
    return Promise.resolve(txn.objectStore(DOC_STORE).index(indexName));
  }
}
  ...
  return new Promise(function (resolve, reject) {
    pdb.get('_design/' + parts[0])
      .then(function (ddoc) {
        ...
        getIndexHandle(pdb, idb, fields, reject)

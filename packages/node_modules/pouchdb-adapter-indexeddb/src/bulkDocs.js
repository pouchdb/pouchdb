...
export default function (api, req, opts, metadata, dbOpts, idbChanges, callback) {
  ...
  function fetchExistingDocs(txn, docs) {
    ...
    docs.forEach(function (doc) {
      const docStore = isLocalId(doc.id) ? META_STORE : DOC_STORE;
  ...
  function write(txn, doc, i) {
    ...
    if (isLocal && doc.deleted) {
      txn.objectStore(META_STORE).delete(doc.id).onsuccess = function () {
        results[i] = {
          ok: true,
          id: doc.id,
          rev: '0-0'
    ...
    const docStore = isLocal ? META_STORE : DOC_STORE;
  ...
  preProcessAttachments().then(function () {
    // REVIEW: We _could_ check doc ids here, and only open the META_STORE if all docs are local.
    // This will marginally slow things down for non-local docs, as they always cause updates to the
    // META_STORE.  It seems pragmatic to keep the code simple and optimise for calls to bulkDocs()

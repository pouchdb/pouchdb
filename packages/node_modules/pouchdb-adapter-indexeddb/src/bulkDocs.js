...
export default function (api, req, opts, metadata, dbOpts, idbChanges, callback) {
  ...
  function write(txn, doc, i) {
    ...
    // Clone and rewrite so that we can index it correctly
    doc.data = rewrite(doc.data);
    // This doesn't make sense here anymore
    // TODO we should clean up this entire function's interaction with these
    // pieces of data to be cleaner. The clone above should be at the start,
    // and we need to go through everywhere to make sure doc.data is only used
    // ever in indexing.

...
function IdbPouch(dbOpts, callback) {
  ...
  // because in the time betweeen getting the db handle and opening the
  ...
  var $t = function (fun, stores, mode) {
    ...
    return function () {
      ...
      setup(openDatabases, api, dbOpts).then(function (res) {
        metadata = res.metadata;
        var txn = res.idb.transaction(stores, mode);
        args.unshift(txn);
        fun.apply(api, args);
      }).catch(function (err) {
        console.error('Failed to establish transaction safely');
        console.error(err);
        // TODO: work out how to surface these usefully to the caller. If I use
        // the commented out code above then some tests fail because of unhandled
        // promise rejections
        // var last = args.unshift();
        // if (typeof last === 'function') {
        //   last(err);
  ...
  // $(function (db, stores, mode, callback) {
  //   try {
  ...
  api._getAttachment = function (docId, attachId, attachment, opts, cb) {
    ...
    // and should be useful. If not then re-work to use $t instead

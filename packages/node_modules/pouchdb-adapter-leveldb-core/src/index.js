  ...
  api._get = readLock(function (id, opts, callback) {
    ...
    stores.docStore.get(id, function (err, metadata) {
      ...
      if (!opts.rev) {
        ...
        rev = opts.rev;
        if (opts.latest) {
          // #7756: If the revision is a valid pattern but it does not exist, this will throw. In this case just
          // continue using the non-existing revision, it will not be found and handled as any other (MISSING_DOC).
          try {
            rev = getLatest(opts.rev, metadata);
          } catch (ex) {
            // swallow exception and let it continue with the non-existing rev that was requested
          }
        }
      }

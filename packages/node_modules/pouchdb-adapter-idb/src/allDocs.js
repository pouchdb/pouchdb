...
function idbAllDocs(opts, idb, callback) {
  ...
  var batchSize = limit === -1 ? -1 : (limit + skip);
  var results = [];
  var docCount;
  ...
  function onBatch(batchKeys, batchValues, cursor) {
    ...
    if (batchSize === -1 || results.length < limit) {
      cursor.continue();
    }
  }
  ...
  function deriveKeyFromValue(value) {
    return value.id;
  }
  ...
  if (!keyRangeError && limit !== 0) {
    ...
    runBatchedCursor(docStore, keyRange, opts.descending, batchSize,
      /* preferSequential */false, /* skipGetAllKeys */true,
      deriveKeyFromValue, onBatch);
  }
}

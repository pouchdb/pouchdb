...
function runBatchedCursor(objectStore, keyRange, descending, batchSize,
                          preferSequential, skipGetAllKeys, deriveKeyFromValue,
                          onBatch) {
  ...
  // TODO: if we're descending and the limit is -1, we can just run a single
  // getAll() and then reverse the results
  ...
  function continuePseudoCursor() {
    if (!valuesBatch.length) { // no more results
      return onBatch();
    }
    // fetch next batch, exclusive start
    var lastKey;
    if (skipGetAllKeys) {
      // if not fetching keys, need a way to derive keys from values
      lastKey = deriveKeyFromValue(valuesBatch[valuesBatch.length - 1]);
    } else {
      lastKey = keysBatch[keysBatch.length - 1];
    }
    var newKeyRange;

...
function findMatchingIndexes(selector, userFields, sortOrder, indexes) {
  return indexes.filter(index => checkIndexMatches(index, sortOrder, userFields, selector));
}
...
function shouldShortCircuit(selector) {
  ...
  const values = Object.keys(selector).map(key => selector[key]);
  return values.some(val => typeof val === 'object' && Object.keys(val).length === 0);
}

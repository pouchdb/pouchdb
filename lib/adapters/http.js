...
function HttpPouch(opts, callback) {
  ...
  api._changeEvents = function (opts) {
    ...
    function done(e) {
      if (!called) {
        api.use_eventsource = false;
        newChanges = api._changes(opts);
      } else {
        //opts.complete(e);
      }
    }
    ...
    source.addEventListener('error', done, false);
    function listener(e) {
      if (called === false) {
        called = true;
      }
    ...
    var out = {
      cancel: function () {
        ...
        source.removeEventListener('error', done, false);
        if (!api.use_eventsource && newChanges) {
          newChanges.cancel();
        }
      }
    };
  ...
  api._changes = function (opts) {
    ...
    if (process.browser && (opts.continuous || opts.live) &&
      global.EventSource && api.use_eventsource === true) {

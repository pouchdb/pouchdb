...
function HttpPouch(opts, callback) {
  ...
  var api = this;
  // If HttpPouch is created with 'call' and a custom 'this' is passed in with its own definition of
  // getHost then that version will be used instead of the one defined in the global context above.
  // This is used to let new adapters wrap http.js and take over getHost processing in order to support
  // their own URL schemes.
...
HttpPouch.destroy = utils.toPromise(function (name, opts, callback) {
  // If an adapter wants to wrap http.js it needs to take over destroy as well. The line below allows the
  // adapter to issue a Javascript 'call' with a custom 'this' that contains a definition for getHost. The
  // next line will recognize that and use the custom getHost, which can handle the adapter's custom URL
  // scheme in favor of the built in getHost defined in the global context above.

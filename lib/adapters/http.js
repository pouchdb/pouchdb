...
=======
  ...
  ajax({
      ...
      if (err) {
        // if error just use old CouchDB version
        return;
      }
      if (resp && resp.version) {
        var tempVersion = parseFloat(resp.version);
        if (typeof tempVersion === 'number')  {
          api._version = tempVersion;
        }
      }
    });
>>>>>>> (#1653) - server sent events for replication

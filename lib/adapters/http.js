...
function getHost(name, opts) {
  ...
  if (/http(s?):/.test(name)) {
    ...
    var parts = uri.pathname.replace(/(^\/|\/$)/g, '').split('/');
    ...
    if (opts.auth) {
      uri.headers.Authorization = 'Basic ' + utils.btoa(opts.auth.username + ':' + opts.auth.password);
    } else if (uri.auth) {
      uri.headers.Authorization = 'Basic ' + utils.btoa(uri.auth);
    }
...
function genDBUrl(opts, path) {
  ...
  if (opts.remote) {
    ...
    return opts.protocol + '://' + opts.host + port + '/' +

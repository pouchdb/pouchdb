  ...
  api._changes = function (opts) {
    ...
    function fetchChanges() {
      ...
      var selectStmt =
        DOC_STORE + '.json AS metadata, ' +
        'MAX(s1.seq) AS seq, ' +
        's2.json AS winningDoc, ' +
        's2.rev AS winningRev ';
      ...
      var criteria = [
        's1.doc_id = ' + DOC_STORE + '.id',
        's1.seq > ' + opts.since,
        's2.seq = ' + DOC_STORE + '.winningseq'
      ];
      ...
      var sql = select(selectStmt, from, null, criteria);

      sql += ' GROUP BY ' + DOC_STORE + '.id' +
        ' ORDER BY seq ' +
        (descending ? 'DESC' : 'ASC');
      ...
      db.readTransaction(function (tx) {
        tx.executeSql(sql, sqlArgs, function (tx, result) {
          ...
          for (var i = 0, l = result.rows.length; i < l; i++) {
            ...
            var doc = unstringifyDoc(res.winningDoc, metadata.id,
              res.winningRev);

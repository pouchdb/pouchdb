...
function WebSqlPouch(opts, callback) {
  ...
  api._doCompaction = function (docId, rev_tree, revs, callback,
    ...
    db.transaction(function (tx) {
      ...
      tx.executeSql(sql, [docId], function (tx, result) {
        ...
        var bySeqKey = "";
        if (recountWinning && metadata.rev_tree.length > 0) {
          var winningRev = merge.winningRev(metadata);
          bySeqKey = metadata.id + "::" + winningRev;
        }

        // websql never calls callback if we do WHERE doc_id_rev IN (...)
        var numDone = 0;

        var sql = recountWinning ? 'UPDATE ' + DOC_STORE +
          ' SET json=?, winningseq=(SELECT seq FROM ' + BY_SEQ_STORE +
          ' WHERE doc_id_rev=?) WHERE id=?' :
          'UPDATE ' + DOC_STORE + ' SET json = ? WHERE id = ?';
        var params = recountWinning ? [JSON.stringify(metadata),
          bySeqKey, docId] : [JSON.stringify(metadata), docId];
        tx.executeSql(sql, params, function () {
  ...
  api._purgeOneRev = function (docId, docRev, callback) {
    api._getRevisionTree(docId, function (err, res) {
      var revTree = res;
      var toRemove = utils.collectParentLeafRevs(revTree[0].ids, docRev, 1, []);
      ...
      if (toRemove[toRemove.length - 1].substr(0, 2) === "1-") {
        api._doCompaction(docId, [], toRemove, function (err2, res2) {
        //now removing whole object from docStore, doCompaction made seqStore
          db.transaction(function (tx) {
            var sql = "DELETE FROM " + DOC_STORE + " WHERE id = ?";
            tx.executeSql(sql, [docId], function () {
        ...
        api._doCompaction(docId, revTree, toRemove, function (err2, res2) {
          if (err2) {
            callback(err2);
          } else {
            callback(null, docRev);
  ...
  api._purge = function (docsList, callback) {
    utils.purge(docsList, callback, api._purgeOneRev);
  };
}

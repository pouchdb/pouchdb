...
var SELECT_DOCS = BY_SEQ_STORE + '.seq AS seq, ' +
  ...
  'hex(' + BY_SEQ_STORE + '.doc_id_rev) AS doc_id_rev, ' +
...
function WebSqlPouch(opts, callback) {
  ...
  function uncompressDoc(doc, id, rev) {
    if (typeof rev === 'undefined') {
      // hex doc_id_rev was passed in instead
      var docIdRev = parseHexString(id, encoding);
      var idx = docIdRev.lastIndexOf('::');
      id = docIdRev.substring(0, idx);
      rev = docIdRev.substring(idx + 2);
    }
  ...
  api._get = function (id, opts, callback) {
    ...
    tx.executeSql(sql, sqlArgs, function (a, results) {
      ...
      doc = uncompressDoc(item.data, item.doc_id_rev);
      finish();
    });
  };
      ...
      countDocs(tx, function (count) {
        ...
        tx.executeSql(sql, sqlArgs, function (tx, result) {
          for (var i = 0, l = result.rows.length; i < l; i++) {
            var item = result.rows.item(i);
            var metadata = JSON.parse(item.metadata);
            var data = uncompressDoc(item.data, item.doc_id_rev);
      ...
      db.readTransaction(function (tx) {
        tx.executeSql(sql, sqlArgs, function (tx, result) {
          ...
          for (var i = 0, l = result.rows.length; i < l; i++) {
            ...
            var doc = uncompressDoc(res.data, res.doc_id_rev);

...
function WebSqlPouch(opts, callback) {
    ...
    function complete(event) {
      var aresults = results
        ...
        .map(function (result) {
          ...
          if (!utils.isLocalId(result.metadata.id)) {
            docsWritten++;
          }
          return {
            ok: true,
            id: result.metadata.id,
            rev: merge.winningRev(result.metadata)
          };
        });
  ...
  api._changes = function (opts) {
    ...
    if (opts.continuous) {
      ...
      WebSqlPouch.Changes.addPouchListener(name, id, api, opts);
      WebSqlPouch.Changes.notify(name);
      return {
        cancel: function () {
          WebSqlPouch.Changes.removePouchListener(name, id);
        }
      };
    }

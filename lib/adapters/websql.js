...
function countDocs(webSqlPouch, tx, callback) {
  if (webSqlPouch.docCount !== -1) {
    return callback(webSqlPouch.docCount);
  }
  ...
  tx.executeSql(sql, [], function (tx, result) {
    webSqlPouch.docCount = result.rows.item(0).num;
    callback(webSqlPouch.docCount);
  });
}

function WebSqlPouch(api, opts, callback) {
  var self = this;
  self.api = api;
  self.instanceId = null;
  self.name = opts.name;
  self.idRequests = [];
  self.docCount = -1; // cache sqlite count(*) for performance

  self.db = openDB(self.name, POUCH_VERSION, self.name, POUCH_SIZE);
  if (!self.db) {
    return callback(errors.UNKNOWN_ERROR);
  } else if (typeof self.db.readTransaction !== 'function') {
    // doesn't exist in sqlite plugin
    self.db.readTransaction = self.db.transaction;
  }

  self.setup(opts, callback);
}

WebSqlPouch.prototype.setup = function (opts, callback) {
  var self = this;

  function dbCreated() {
    // note the db name in case the browser upgrades to idb
    if (utils.hasLocalStorage()) {
      global.localStorage['_pouch__websqldb_' + self.name] = true;
    }
  ...
  function onGetInstanceId(tx) {
    while (self.idRequests.length > 0) {
      var idCallback = self.idRequests.pop();
      idCallback(null, self.instanceId);
    }
    checkDbEncoding(tx);
  }
  ...
  function checkDbEncoding(tx) {
    ...
    tx.executeSql('SELECT dbid, hex(dbid) AS hexId FROM ' + META_STORE, [],
      function (err, result) {
        ...
        self.encoding = (hexId.length === id.length * 2) ? 'UTF-8' : 'UTF-16';
      }
    );
  }
  ...
  function onGetVersion(tx, dbVersion) {
    if (dbVersion === 0) {
      ...
      tx.executeSql(meta, [], function () {
        ...
        self.instanceId = utils.uuid();
        tx.executeSql(initSeq, [0, ADAPTER_VERSION, self.instanceId]);
        onGetInstanceId(tx);
      });
    } else { // version > 0
      ...
      tx.executeSql('SELECT dbid FROM ' + META_STORE, [],
        function (tx, result) {
          self.instanceId = result.rows.item(0).dbid;
          onGetInstanceId(tx);
        });
    }
  }
  ...
  if (utils.isCordova() && typeof global !== 'undefined') {
    ...
    global.addEventListener(self.name + '_pouch', function cordova_init() {
      global.removeEventListener(self.name + '_pouch', cordova_init, false);
      setup();
    }, false);
  } else {
    setup();
...
WebSqlPouch.prototype.type = function () {
  return 'websql';
};

WebSqlPouch.prototype._id = utils.toPromise(function (callback) {
  var self = this;

  callback(null, self.instanceId);
});

WebSqlPouch.prototype._info = function (callback) {
  var self = this;

  self.db.readTransaction(function (tx) {
    countDocs(self, tx, function (docCount) {
...
WebSqlPouch.prototype._bulkDocs = function (req, opts, callback) {
  var self = this;
  ...
  function complete(event) {
    ...
    results.forEach(function (result) {
      ...
      WebSqlProto.Changes.notify(self.name);
      WebSqlProto.Changes.notifyLocalWindows(self.name);
    });
  ...
  function metadataFetched(tx, results) {
    for (var j = 0; j < results.rows.length; j++) {
      ...
      var id = parseHexString(row.hexId, self.encoding);
  ...
  preprocessAttachments(function () {
    self.db.transaction(function (txn) {
      tx = txn;
...
WebSqlPouch.prototype._get = function (id, opts, callback) {
  var self = this;
  ...
  var err;
  if (!opts.ctx) {
    self.db.readTransaction(function (txn) {
      opts.ctx = txn;
      self._get(id, opts, callback);
    });
    return;
  }
  var tx = opts.ctx;
...
WebSqlPouch.prototype._allDocs = function (opts, callback) {
  var self = this;
  ...
  self.db.readTransaction(function (tx) {
    ...
    countDocs(self, tx, function (count) {
      totalRows = count;
...
WebSqlPouch.prototype._changes = function (opts) {
  var self = this;
  opts = utils.clone(opts);

  if (opts.continuous) {
    var id = self.name + ':' + utils.uuid();
    WebSqlProto.Changes.addListener(self.name, id, self.api, opts);
    WebSqlProto.Changes.notify(self.name);
    return {
      cancel: function () {
        WebSqlProto.Changes.removeListener(self.name, id);
      }
    };
  }
...
WebSqlPouch.prototype._close = function (callback) {
  //WebSQL databases do not need to be closed
  callback();
};

WebSqlPouch.prototype._getAttachment = function (attachment, opts, callback) {
  var self = this;
  ...
  tx.executeSql(sql, [digest], function (tx, result) {
    ...
    var data = parseHexString(result.rows.item(0).body, self.encoding);
    if (opts.encode) {
      res = btoa(data);
    } else {
...
WebSqlPouch.prototype._getRevisionTree = function (docId, callback) {
  var self = this;
...
WebSqlPouch.prototype._doCompaction =
    function (docId, rev_tree, revs, callback) {
  var self = this;
...
function WebSqlProto(opts, callback) {
  var api = this;
  var webSqlPouch = new WebSqlPouch(api, opts, callback);
  Object.keys(WebSqlPouch.prototype).forEach(function (key) {
    api[key] = utils.getArguments(function (args) {
      WebSqlPouch.prototype[key].apply(webSqlPouch, args);
    });
  });
}

WebSqlProto.valid = function () {
  if (typeof global !== 'undefined') {
    if (global.navigator &&
...
WebSqlProto.destroy = utils.toPromise(function (name, opts, callback) {
...
WebSqlProto.Changes = new utils.Changes();

module.exports = WebSqlProto;

...
function WebSqlPouch(opts, callback) {
      ...
      tx.executeSql('ALTER TABLE ' + DOC_STORE +
        ...
        tx.executeSql(sql, [], function (tx, result) {
          ...
          for (var i = 0; i < result.rows.length; i++) {
            ...
            // SQLite Plugin sometimes gives strings (#2434)
            var seq = parseInt(item.seq, 10);
      ...
      tx.executeSql('SELECT sql FROM sqlite_master WHERE tbl_name = ' +
        ...
        if (!result.rows.length) {
          ...
          tx.executeSql('SELECT db_version FROM ' + META_STORE, [],
            function (tx, result) {
            // SQLite Plugin sometimes gives strings (#2434)
            var dbVersion = parseInt(result.rows.item(0).db_version, 10);
  ...
  api._info = function (callback) {
    db.readTransaction(function (tx) {
      countDocs(tx, function (docCount) {
        ...
        tx.executeSql(sql, [], function (tx, result) {
          // SQLite Plugin sometimes gives strings (#2434)
          var updateSeq = parseInt(result.rows.item(0).update_seq, 10);
          callback(null, {
            doc_count: docCount,
  ...
  api._bulkDocs = function (req, opts, callback) {
    ...
    function complete() {
      ...
      tx.executeSql(updateseq, [], function (tx, result) {
        // SQLite Plugin sometimes gives strings (#2434)
        var oldUpdateSeq = parseInt(result.rows.item(0).update_seq, 10);
        var update_seq = oldUpdateSeq + updateSeq;
      ...
      function finish() {
        ...
        tx.executeSql(sql, sqlArgs, function (tx, result) {
          ...
          tx.executeSql(fetchSql, [doc_id_rev], function (tx, res) {
            // SQLite Plugin sometimes gives strings (#2434)
            var seq = parseInt(res.rows.item(0).seq, 10);
  ...
  api._get = function (id, opts, callback) {
    ...
    tx.executeSql(sql, sqlArgs, function (a, results) {
      ...
      // SQLite Plugin sometimes gives strings (#2434)
      var deleted = parseInt(item.deleted, 10);
      if (deleted && !opts.rev) {
  ...
  function countDocs(tx, callback) {
    ...
    tx.executeSql(sql, [], function (tx, result) {
      // SQLite Plugin sometimes gives strings (#2434)
      docCount = parseInt(result.rows.item(0).num, 10);
      callback(docCount);
    });
  }

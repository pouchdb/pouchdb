...
function WebSqlPouch(opts, callback) {
  ...
  function uncompressDoc(doc, id, rev) {
  ...
  /*function runMigration4(tx, callback) {
    //var sql = 'ALTER TABLE ' + BY_SEQ_STORE + ' ADD COLUMN rev UNIQUE';
    callback();
  }*/
  ...
  function onGetVersion(tx, dbVersion) {
    if (dbVersion === 0) {
      ...
      var seq = 'CREATE TABLE IF NOT EXISTS ' + BY_SEQ_STORE +
        ...
        'json, deleted TINYINT(1), doc_id INTEGER, rev, ' +
        'FOREIGN KEY(doc_id) REFERENCES ' + DOC_STORE + '(rowid))';
      var local = 'CREATE TABLE IF NOT EXISTS ' + LOCAL_STORE +
        ' (id UNIQUE, rev, json)';
  ...
  api._bulkDocs = function (req, opts, callback) {
    ...
    function writeDoc(docInfo, winningRev, deleted, callback, isUpdate) {
      ...
      function finish() {
        ...
        var id = data._id;
        var rev = data._rev;
        var json = compressDoc(data);
        var docIdSubquery = isUpdate ?
          'SELECT rowid FROM ' + DOC_STORE + ' WHERE id=?' :
          'SELECT 1 + IFNULL(MAX(rowid), 0) FROM ' + DOC_STORE;
        var sql = 'INSERT INTO ' + BY_SEQ_STORE +
          ' (doc_id, rev, json, deleted) VALUES ((' +
          docIdSubquery + '), ?, ?, ?);';
        var sqlArgs = isUpdate ?
          [id, rev, json, deletedInt] :
          [rev, json, deletedInt];
        ...
        tx.executeSql(sql, sqlArgs, function (tx, result) {
          ...
          var docIdUpdateSubquery = 'SELECT rowid FROM ' + DOC_STORE +
          ' WHERE id=?';
          var fetchSql = select('seq', BY_SEQ_STORE, null,
            'doc_id=(' + docIdUpdateSubquery + ') AND rev=?');
          tx.executeSql(fetchSql, [id, rev], function (tx, res) {
            var seq = res.rows.item(0).seq;
            var sql = 'UPDATE ' + BY_SEQ_STORE +
              ' SET json=?, deleted=? WHERE doc_id=(' +
              docIdUpdateSubquery + ') AND rev=?;';
      ...
      function dataWritten(tx, seq) {
        ...
        var sql = isUpdate ?
          ...
          ' WHERE doc_id=' + DOC_STORE + '.rowid AND rev=?) WHERE id=?'
  ...
  api._get = function (id, opts, callback) {
    ...
    if (opts.rev) {
      sql = select(
        ...
        DOC_STORE + '.rowid=' + BY_SEQ_STORE + '.doc_id',
        [DOC_STORE + '.id=?', BY_SEQ_STORE + '.rev=?']);
      sqlArgs = [id, opts.rev];
    } else {
      sql = select(
        SELECT_DOCS,
    ...
    tx.executeSql(sql, sqlArgs, function (a, results) {
      ...
      doc = uncompressDoc(item.data, metadata.id, item.rev);
      finish();
    });
  };
      ...
      countDocs(tx, function (count) {
        ...
        tx.executeSql(sql, sqlArgs, function (tx, result) {
          for (var i = 0, l = result.rows.length; i < l; i++) {
            var item = result.rows.item(i);
            var metadata = JSON.parse(item.metadata);
            var data = uncompressDoc(item.data, metadata.id, item.rev);
      ...
      db.readTransaction(function (tx) {
        tx.executeSql(sql, sqlArgs, function (tx, result) {
          ...
          for (var i = 0, l = result.rows.length; i < l; i++) {
            ...
            var doc = uncompressDoc(res.data, metadata.id, res.rev);
  ...
  api._doCompaction = function (docId, rev_tree, revs, callback) {
    ...
    db.transaction(function (tx) {
      var sql = 'SELECT json AS metadata, rowid AS rowid FROM ' +
        DOC_STORE + ' WHERE id = ?';
      tx.executeSql(sql, [docId], function (tx, result) {
        if (!result.rows.length) {
        ...
        revs.forEach(function (rev) {
          ...
          tx.executeSql(sql, [rowid, rev], function (tx) {
    ...
    db.readTransaction(function (tx) {
      ...
      tx.executeSql(sql, [id], function (tx, res) {
        if (res.rows.length) {
          ...
          var doc = uncompressDoc(item.json, id, item.rev);
          callback(null, doc);
        } else {
          callback(errors.MISSING_DOC);
        }

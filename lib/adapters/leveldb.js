...
function LevelPouch(opts, callback) {
  ...
  var uuidPath = path.join(opts.name, '.uuid');
  ...
  api.destroy = utils.toPromise(function (opts, callback) {
    ...
    function done() {
      ...
      var altUuidPath = path.join(name, '.uuid');
      fs.unlink(uuidPath, function (err) {
        if (err) {
          fs.unlink(altUuidPath, function (err) {
            if (err) {
...
LevelPouch.destroy = utils.toPromise(function (name, opts, callback) {
  ...
  if (fs.existsSync(opts.name + '.uuid') || fs.existsSync(path.join(opts.name, '.uuid'))) {

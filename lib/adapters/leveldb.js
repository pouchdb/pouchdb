  ...
  api._allDocs = function (opts, callback) {
    ...
    totalRowsStream.on('close', function () {
      ...
      if ('startkey' in opts && opts.startkey) {
        if (readstreamOpts.reverse) {
          readstreamOpts.end = opts.startkey;
      ...
      if ('endkey' in opts && opts.endkey) {
        if (readstreamOpts.reverse) {
          readstreamOpts.start = opts.endkey;
        } else { // ascending order
      ...
      if ('start' in readstreamOpts && 'end' in readstreamOpts) {
        if (readstreamOpts.start > readstreamOpts.end) {
          // impossible to return results b/c start is greater than end.
          // normally level would "fix" this for us
          // by reversing the order, but Couch would return 0 results here
          return call(callback, null, {

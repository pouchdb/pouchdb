...
function LevelPouch(opts, callback) {
  ...
  api._bulkDocs = writeLock(function (req, opts, callback) {
    ...
    function writeDoc(doc, index, callback2) {
      ...
      function finish() {
        ...
        // TODO: we are not necessarily caching the winning seq, figure
        // out what metadata.seq is needed for
  ...
  api._changes = function (opts) {
    ...
    var emittedDocIds = new utils.Set();
    ...
    var throughStream = through(function (data, _, next) {
      ...
      stores.docStore.get(data.value._id, function (err, metadata) {
        ...
        function onGetWinningDoc(winningDoc) {
          ...
          if (filter(change)) {
            ...
            if (opts.attachments && opts.include_docs) {
              ...
              // of live listeners
              return fetchAttachments([change], stores).then(function () {
        ...
        emittedDocIds.add(doc._id);
        if (seq === winningSeq) {
          onGetWinningDoc(doc);
        } else { // nope, need to fetch some other doc
          stores.bySeqStore.get(formatSeq(winningSeq),
              function (err, winningDoc) {
            onGetWinningDoc(winningDoc);
          });
        }
      });
      ...
      if (returnDocs) {
        if (opts.limit && opts.limit < results.length) {

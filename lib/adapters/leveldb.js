...
function LevelPouch(opts, callback) {
  ...
  api._purgeOneRev = function (docId, docRev, callback) {
    stores.docStore.get(docId, function (err, res) {
      ...
      var toRemove = utils.collectParentLeafRevs(revTree, docRev, 1, []);
      var newRevTree = utils.updateRevTree(res.rev_tree, docRev);
      console.log("newrevTree" + newRevTree);
      ...
      api._doCompaction(docId, newRevTree, toRemove, function (err2, res2) {
        if (err2) {
          callback(err2);
          return;
        } else {
          if (newRevTree[0]) { //todo:
            callback(null, docRev);
            return;
          }
          console.log("purging whole doc");
          stores.docStore.del(docId, function (err3, res3) {
            console.log("deleting", err3, res3);
            if (err3) {
              callback(err3);
            } else {
              console.log("finish leveldb");
              callback(null, docRev);
            }
            return;
          });
  ...
  api._purge = function (docsList, callback) {
    utils.purge(docsList, callback, api._purgeOneRev, this);
  };
}

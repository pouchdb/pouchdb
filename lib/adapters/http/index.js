...
function HttpPouch(opts, callback) {
  ...
  api._bulkDocs = function (req, opts, callback) {
    ...
    setup().then(function () {
      ...
      var hasNonStubAttachments = !req.docs.every(function (doc) {
        var atts = doc._attachments;
        var hasAtts = atts && Object.keys(atts).filter(function (att) {
          return !atts[att].stub;
        }).length;
        return !hasAtts;
      });
      var ajaxOpts = {
      ...
      if (hasNonStubAttachments) {
        ...
        var multipart = createMultipart(req);
      ...
      ajax(opts, ajaxOpts, function (err, results) {
        if (err) {
          return callback(err);
        }

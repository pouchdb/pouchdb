...
var debug = require('debug')('pouchdb:adapter-leveldb');
...
function LevelPouch(opts, callback) {
  ...
  api._getAttachment = function (attachment, opts, callback) {
    ...
    stores.binaryStore.get(digest, function (err, attach) {
      ...
      stores.attachmentStore.get(digest, function (err, attachmentMeta) {
        if (err) {
          return callback(err);
        }

        debug('GET %j', attachmentMeta);
        callback(null, {data: data, revPos: attachmentMeta.revPos});
      });
    });
  };
  ...
  api._bulkDocs = writeLock(function (req, opts, callback) {
    ...
    function writeDoc(doc, winningRev, deleted, callback2,
      ...
      for (var i = 0; i < attachments.length; i++) {
        ...
        if (att.stub) {
          ...
          saveAttachmentRefs(id, rev, att.revPos, att.digest, attachmentSaved);
          continue;
        }
        var data;
    ...
    function saveAttachmentRefs(id, rev, revPos, digest, callback) {
      ...
      function saveAtt(oldAtt) {
        ...
        if (oldAtt) {
          ...
          newAtt.refs = {};
          newAtt.refs[ref] = true;
          debug('revPos %s', revPos);
          newAtt.revPos = revPos;
        }
    ...
    function saveAttachment(docInfo, digest, key, data, callback) {
      ...
      var revPos = utils.getRevPath(rev, docInfo.metadata.rev_tree)
          .pos - 1;

      saveAttachmentRefs(id, rev, revPos, digest,
        function (err, isNewAttachment) {
          if (err) {
            return callback(err);
          }
          ...
          txn.get(stores.attachmentStore, digest, function (err, attData) {
            ...
            if (refs.length) { // not orphaned
              batch.push({
                ...
                type: 'put',
                valueEncoding: 'json',
                value: {refs: newRefs, revPos: attData.revPos},
                prefix: stores.attachmentStore
              });

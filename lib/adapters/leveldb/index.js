...
function LevelPouch(opts, callback) {
  ...
  if (dbStore.has(name)) {
    ...
    console.log('calling levelup() with args:', name, opts);
    dbStore.set(name, sublevel(levelup(name, opts, function (err) {
      console.log('called levelup()');
      if (err) {
        console.log('error opening levelup', err);
  ...
  api._destroy = function (callback) {
    ...
    callback = function (err, res) {
      if (err) {
        console.log('error destroying leveldown', err);
        console.log(err.stack);
      }
      oldCallback(err, res);
    };

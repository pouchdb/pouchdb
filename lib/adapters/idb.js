  ...
  api._bulkDocs = function idb_bulkDocs(req, opts, callback) {
    ...
    function fetchExistingDocs(callback) {
      ...
      for (var i = 0, len = docInfos.length; i < len; i++) {
        ...
        if (docInfo.metadata) {
          nonLocalIds.push(docInfo.metadata.id);
        }
      }

      if (!nonLocalIds.length) {
        return callback();
      }
      ...
      var keyIdx = 0;
      var currentKey = nonLocalIds[keyIdx];
      var cursor = txn.objectStore(DOC_STORE).openCursor(
        global.IDBKeyRange.lowerBound(currentKey));

      cursor.onsuccess = function process_docRead(e) {
        cursor = e.target.result;
        if (!cursor) {
          return callback();
        }
        if (currentKey === cursor.primaryKey) {
          fetchedDocs.set(cursor.primaryKey, decodeMetadata(cursor.value));
        }
        while (currentKey <= cursor.primaryKey &&
               keyIdx < nonLocalIds.length - 1) {
          currentKey = nonLocalIds[++keyIdx];
        }

        if (currentKey > cursor.primaryKey) {
          cursor.continue(currentKey);
        } else {
          callback();
        }
      };

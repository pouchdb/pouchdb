...
function IdbPouch(opts, callback) {
  ...
  function addPluginData(e) {
    var db = e.currentTarget;

    var pluginStore = db.createObjectStore(PLUGIN_DATA_STORE, {keyPath : 'primaryKey'});
    pluginStore.createIndex('name', {keyPath : 'name'});

    // TODO: IE doesn't accept complex keys, so we'll have to use indexable_string
    pluginStore.createIndex('nameAndId', {keyPath : 'nameAndId'});
    pluginStore.createIndex('nameAndKey', {keyPath : 'nameAndKey'});
  }
  ...
  function putNewKeyValues() {
    ...
    Object.keys(keyValues).forEach(function (key) {
      ...
      store.put({
        ...
        key : key,
        value : value,
        nameAndId : [self.name, id],
        nameAndKey : [self.name, key]
      });
    });
  }

  index.openCursor(global.IDBKeyRange.only([self.name, id])).onsuccess = function (event) {
    var cursor = event.target.result;
...
Index.prototype.get = utils.toPromise(function (start, end, callback) {
  ...
  if (typeof start !== 'undefined' && typeof start !== 'string') {
    return callback(errors.error(errors.IDB_ERROR, 'start must be a string if defined',
      'start-must-be-string'));
  }
  if (typeof end !== 'undefined' && typeof end !== 'string') {
    return callback(errors.error(errors.IDB_ERROR, 'end must be a string if defined',
      'end-must-be-string'));
  }

  var results = [];
  var txn = self.idb.transaction([PLUGIN_DATA_STORE], 'readonly');
  var index = txn.objectStore(PLUGIN_DATA_STORE).index('nameAndKey');

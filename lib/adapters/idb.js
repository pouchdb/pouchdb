  ...
  revs.forEach(function (rev) {
    ...
    index.getKey(key).onsuccess = function (e) {
      ...
      var cursor = attAndSeqStore.index('seq')
        .openCursor(window.IDBKeyRange.only(seq));
...
function init(api, opts, callback) {
    ...
    cursor.onsuccess = function (event) {
      ...
      if (cursor) {
        ...
        if (local) {
          ...
          var end = docId + "::~";
          var index = seqStore.index('_doc_id_rev');
          var range = window.IDBKeyRange.bound(start, end, false, false);
    ...
    cursor.onsuccess = function (e) {
      ...
      function fetchMetadataSeq() {
        ...
        var req = bySeqStore.index('_doc_id_rev').openCursor(
          window.IDBKeyRange.bound(start, end));
  ...
  function allDocsQuery(totalRows, opts, callback) {
    ...
    try {
      if (start && end) {
        keyRange = window.IDBKeyRange.bound(start, end, false, !inclusiveEnd);
      } else if (start) {
        if (descending) {
          keyRange = window.IDBKeyRange.upperBound(start);
        } else {
          keyRange = window.IDBKeyRange.lowerBound(start);
        }
      } else if (end) {
        if (descending) {
          keyRange = window.IDBKeyRange.lowerBound(end, !inclusiveEnd);
        } else {
          keyRange = window.IDBKeyRange.upperBound(end, !inclusiveEnd);
        }
      } else if (key) {
        keyRange = window.IDBKeyRange.only(key);
      }
    } catch (e) {
  ...
  function countDocs(callback) {
    ...
    index.count(window.IDBKeyRange.only("0")).onsuccess = function (e) {
  ...
  api._changes = function (opts) {
    ...
    function onGetCursor(cursor) {
      ...
      function fetchWinningDoc() {
        ...
        var req = bySeqStore.index('_doc_id_rev').openCursor(
          window.IDBKeyRange.bound(docIdRev, docIdRev + '\uffff'));
        req.onsuccess = function (e) {
    ...
    function fetchChanges() {
      ...
      if (descending) {
        ...
        req = bySeqStore.openCursor(
          window.IDBKeyRange.lowerBound(opts.since, true));
      }
  ...
  var req = window.indexedDB.open(name, ADAPTER_VERSION);
...
IdbPouch.valid = function () {
  ...
  return !isSafari && window.indexedDB && window.IDBKeyRange;
};
...
function destroy(name, opts, callback) {
  ...
  req.onsuccess = function () {
    ...
    if (utils.hasLocalStorage() && (name in window.localStorage)) {

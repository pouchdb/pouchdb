...
function init(api, opts, callback) {
  ...
  api._changes = function (opts) {
    ...
    function onGetCursor(cursor) {
      ...
      var metadata = decodeMetadata(cursor.value);
      var seq = cursor.key;
      var docId = cursor.primaryKey;

      lastSeq = seq;

      if (docIds && !docIds.has(docId)) {
      ...
      function fetchWinningDoc(winningRev) {
        var docIdRev = docId + '::' + winningRev;
        var req = docIdRevIndex.get(docIdRev);
        req.onsuccess = function (e) {
          onGetWinningDoc(e.target.result);
        };
      }
    ...
    function fetchChanges() {
      ...
      maxSeqIndex = docStore.index('seq');

      var req;

      if (descending) {
        // Ignore the `since` parameter when `descending` is true
        req = maxSeqIndex.openCursor(null, descending);
      } else {
        req = maxSeqIndex.openCursor(
  ...
  req.onupgradeneeded = function (e) {
    ...
    if (e.oldVersion < 5) {
      if (e.oldVersion < 4) {
        addAttachAndSeqStore(db); // v4 -> v5
        if (e.oldVersion < 3) {
          createLocalStoreSchema(db); // v3 -> v4
          if (e.oldVersion < 2) {
            addDeletedOrLocalIndex(txn, function () { // v1 -> v2
              migrateLocalStore(txn, function () { // v2 -> v3
                migrateAttsAndSeqs(txn, function () { // v3 -> v4
                  migrateMetadata(txn); // v4 -> v5
                });
              });
            });
          } else {

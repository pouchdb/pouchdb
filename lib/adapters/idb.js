...
Index.prototype.get = utils.toPromise(function (start, end, callback) {
  ...
  var txn = self.idb.transaction([PLUGIN_DATA_STORE], 'readonly');
  var store = txn.objectStore(PLUGIN_DATA_STORE);
  var index = store.index((hasStart || hasEnd) ? 'nameAndKey' : 'name');

  var keyRange = hasStart && hasEnd ? global.IDBKeyRange.bound([self.name, start], [self.name, end])
    : hasStart ? global.IDBKeyRange.lowerBound([self.name, start])
    : hasEnd ? global.IDBKeyRange.upperBound([self.name, end])

...
function init(api, opts, callback) {
  ...
  api._changes = function (opts) {
    ...
    function fetchChanges() {
      ...
      /*bySeqStore.openCursor(null, 'prev').onsuccess = function (event) {
        var cursor = event.target.result;
        if (cursor) {
          updateSeq = cursor.key;
        } else {
    ...
    function onsuccess(event) {
      ...
      docStore.get(doc._id).onsuccess = function (event) {
        ...
        function onGetWinningDoc(winningDoc, winningSeq) {
          delete winningDoc['_doc_id_rev'];

          var change = opts.processChange(winningDoc, metadata, opts);
          change.seq = seq < winningSeq ? seq : winningSeq;
          if (filter(change)) {
            numResults++;
            emittedDocIds.add(winningDoc._id);
            if (returnDocs) {
              results.push(change);
            }
        ...
        if (winningRev === doc._rev) {
          // the succeeding revs don't matter
          return onGetWinningDoc(doc, seq);
        }
        //
        // TODO: implement rev_map instead
        //
        // fetch the winner
        ...
        req.onsuccess = function (e) {
          var winningSeq = e.target.result.primaryKey;
          if (winningSeq > seq) {
            // wait until later
            return cursor.continue();
          }

          onGetWinningDoc(e.target.result.value, winningSeq);
        };
      };
    }

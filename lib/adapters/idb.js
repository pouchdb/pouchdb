...
Index.prototype.get = utils.toPromise(function (opts, callback) {
  ...
  try {
    keyRange = hasStart && hasEnd ? global.IDBKeyRange.bound(
      ...
      : hasStart ? (descending ?
          global.IDBKeyRange.upperBound([self.name, start])
        : global.IDBKeyRange.lowerBound([self.name, start]))
      : hasEnd ? (descending ?
          global.IDBKeyRange.lowerBound([self.name, end])
        : global.IDBKeyRange.upperBound([self.name, end]))
      : hasKey ? global.IDBKeyRange.only([self.name, key])

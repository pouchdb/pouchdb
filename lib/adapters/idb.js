...
function init(api, opts, callback) {
  ...
  api._bulkDocs = function idb_bulkDocs(req, opts, callback) {
    ...
    function fetchExistingDocs(callback) {
      ...
      docInfos.forEach(function (docInfo) {
        ...
        console.time('process_docRead : ' + id);
        req.onsuccess = function process_docRead(event) {
          console.timeEnd('process_docRead : ' + id);
    ...
    function writeDoc(docInfo, winningRev, deleted, callback, _up, resultsIdx) {
      ...
      function finish() {
        ...
        function afterPut(e) {
          console.timeEnd('putReq : ' + docInfo.metadata.id);
          ...
          console.time('metaDatareq : ' + metadata.seq);
          metaDataReq.onsuccess = function () {
            console.timeEnd('metaDatareq : ' + metadata.seq);
        ...
        console.time('putReq : ' + docInfo.metadata.id);
  ...
  function allDocsQuery(totalRows, opts, callback) {
    ...
    var results = [];
    oCursor.onsuccess = function allDocsCursor(e) {
      if (!e.target.result) {
        return;
      }
      var cursor = e.target.result;
      ...
      if (!opts.include_docs) {
        ...
        console.time('fetch winning rev: ' + key);
        index.get(key).onsuccess = function allDocsGet(event) {
          console.timeEnd('fetch winning rev: ' + key);
  ...
  api._changes = function (opts) {
    ...
    function onsuccess(event) {
      ...
      console.time('changes get: ' + doc._id);
      index.get(doc._id).onsuccess = function (event) {
        console.timeEnd('changes get: ' + doc._id);
        var metadata = decodeMetadata(event.target.result);
  ...
  api._getLocal = function (id, callback) {
    ...
    console.time('getLocal: ' + id);
    req.onsuccess = function (e) {
      console.timeEnd('getLocal: ' + id);
  ...
  api._putLocal = function (doc, opts, callback) {
    ...
    if (oldRev) {
      ...
      console.time('putLocal 1: ' + id);
      req.onsuccess = function (e) {
        console.timeEnd('putLocal 1: ' + id);
        if (!e.target.result) {
          callback(errors.REV_CONFLICT);
        } else { // update
          var req = oStore.put(doc);
          console.time('putLocal 2: ' + id);
          req.onsuccess = function () {
            console.timeEnd('putLocal 2: ' + id);
      ...
      req = oStore.get(id);
      console.time('putLocal 3: ' + id);
      req.onsuccess = function (e) {
        console.timeEnd('putLocal 3: ' + id);
        if (e.target.result) { // already exists
          callback(errors.REV_CONFLICT);
        } else { // insert
          var req = oStore.put(doc);
          console.time('putLocal 4: ' + id);
          req.onsuccess = function () {
            console.timeEnd('putLocal 4: ' + id);
            ret = {ok: true, id: doc._id, rev: doc._rev};

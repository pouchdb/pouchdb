...
function idbBulkDocs(req, opts, api, idb, Changes, callback) {
  ...
  function addSecondaryIndexes(metadataToStore, doc) {
    if (!opts.indexers) {
      return;
    }
    Object.keys(opts.indexers).forEach(function (indexName) {
      var indexer = opts.indexers[indexName];
      var keyValues = indexer(doc);
      keyValues.forEach(function (keyValue) {
        metadataToStore.idx.push(indexName + '\u0000' + keyValue.key);
        if (keyValue.value) {
          metadataToStore.idxValues = metadataToStore.idxValues || {};
          metadataToStore.idxValues[keyValue.key] = keyValue.value;
        }
      });
    });
  }
  ...
  function finishDoc(docInfo, winningRev, winningRevIsDeleted,
    ...
    function afterPutDoc(e) {
      ...
      var metadataToStore = encodeMetadata(metadata, winningRev,
        winningRevIsDeleted);
      addSecondaryIndexes(metadataToStore, doc);

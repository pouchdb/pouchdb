...
var encodeWinningSeqs = idbUtils.encodeWinningSeqs;

function cloneSparseArray(arr) {
  var out = [];
...
function idbBulkDocs(req, opts, api, idb, Changes, callback) {
  ...
  var metaStore;
  var docInfoError;
  var newWinningSeqs = cloneSparseArray(api._winningSeqs);
  ...
  function complete() {
    ...
    api._winningSeqs = newWinningSeqs;
  ...
  function updateWinningSeqs() {
    metaStore.put({
      id: WINNING_SEQS_KEY,
      winningSeqs: encodeWinningSeqs(newWinningSeqs)
    });
  }
  ...
  function finishDoc(docInfo, winningRev, deleted, callback, isUpdate,
    ...
    function afterPutDoc(e) {
      ...
      var newSeq = e.target.result;
      if (metadata.seq) {
        delete newWinningSeqs[metadata.seq];
      }
      newWinningSeqs[newSeq] = true;
      metadata.seq = newSeq;

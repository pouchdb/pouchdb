...
var debug = require('debug')('pouchdb:adapter-idb');
...
function idbBulkDocs(req, opts, api, idb, Changes, callback) {
  ...
  function writeAttachments(docInfo, winningRev, deleted, callback,
    ...
    attachments.forEach(function (key) {
      ...
      if (!att.stub) {
        ...
        var currentRev = docInfo.metadata.rev;
        var revPos = utils.getRevPath(currentRev, docInfo.metadata.rev_tree)
          .ids.length - 1;

        saveAttachment(revPos, digest, data, attachmentSaved);
      } else {
        numDone++;
        collectResults();
      }
  ...
  function saveAttachment(revPos, digest, data, callback) {

    debug('PUT attachment revPos %s', revPos);

...
function idbBulkDocs(req, opts, api, idb, Changes, callback) {
  ...
  var docInfoError;
  var newWinningSeqs = new utils.Map();
  var losingSeqs = [];
  ...
  function complete() {
    ...
    newWinningSeqs.forEach(function (winningSeq) {
      api._winningSeqs[winningSeq] = true;
    });

    losingSeqs.forEach(function (losingSeq) {
      delete api._winningSeqs[losingSeq];
    });
  ...
  function finishDoc(docInfo, winningRev, deleted, callback, isUpdate,
    ...
    function afterPutDoc(e) {
      ...
      if (metadata.seq) {
        losingSeqs.push(metadata.seq);
      }
      newWinningSeqs.set(metadata.id, newSeq);
      metadata.seq = newSeq;

...
var hasGetAll = idbUtils.hasGetAll;
var getAll = idbUtils.getAll;
...
function idbAllDocs(opts, api, idb, callback) {
  ...
  function allDocsQuery(opts, callback) {
    ...
    function allDocsInner(winningRev, metadata, cursor) {
      var row = {
        id: metadata.id,
        key: metadata.id,
        value: {
    ...
    function onGetCursor(e) {
      ...
      allDocsInner(winningRev, metadata, cursor);
    }
    ...
    // Use getAll() if it's available, because it's faster (#4235).
    if (hasGetAll(docStore)) {
      var idbLimit = limit === -1 ? null : limit;
      if (idbLimit !== null && skip) {
        idbLimit += skip;
      }
      req = getAll(docStore, keyRange, idbLimit);
      req.onsuccess = function (e) {
        docCount = api._meta.docCount; // do this within the txn for consistency
        var resultList = e.target.result;
        if (descending) {
          resultList = resultList.reverse();
        }
        if (skip) {
          resultList = resultList.slice(skip);
          skip = 0;
        }
        for (var i = 0; i < resultList.length; i++) {
          var metadata = decodeMetadata(resultList[i]);
          var winningRev = metadata.winningRev;

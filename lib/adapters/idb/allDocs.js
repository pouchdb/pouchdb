...
function idbAllDocs(opts, api, idb, callback) {
  ...
  function allDocsQuery(opts, callback) {
    ...
    var indexToUse = docStore;

    if (opts.index) {
      indexToUse = docStore.index('idx');
      if (start !== false) {
        start = opts.index + '\u0000' + start;
      }
      if (end !== false) {
        end = opts.index + '\u0000' + end;
      }
    ...
    function startCursor() {
      var cursor = descending ?
        indexToUse.openCursor(keyRange, descending) :
        indexToUse.openCursor(keyRange);

      cursor.onsuccess = onGetCursor;
    }

    if (opts.index) {
      // dynamically count all keys in the index
      var counter = indexToUse.count();
      counter.onsuccess = function (e) {
        docCount = e.target.result;
        startCursor();
      };
    } else {
      // can use the cached value in api._docCount
      startCursor();
    }
  }

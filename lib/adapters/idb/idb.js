...
function init(api, opts, callback) {
  ...
  function initIdb() {
      ...
      function onGetMetadata(meta) {
        ...
        function createBlobSupportPromise() {
          ...
          return new utils.Promise(function (resolve) {
            ...
            return false; // something went wrong, assume not supported
          });
        }
    ...
    var openReq = indexedDB.open(name, ADAPTER_VERSION);

    if (!('openReqList' in IdbPouch)) {
      IdbPouch.openReqList = new utils.Map();
    }
    IdbPouch.openReqList.set(name, openReq);
...
function destroy(name, opts, callback) {
  ...
  var openReq = IdbPouch.openReqList.get(name);
  ...
  req.onsuccess = function () {
    ...
    if (IdbPouch.openReqList.has(name)) {
      IdbPouch.openReqList.delete(name);
    }

...
function destroy(name, opts, callback) {
  ...
  // do a dummy transaction, so that it doesn't destroy the database
  // before other concurrent writes are ready
  var txnResult = openTransactionSafely(cached.idb,
    [DOC_STORE, BY_SEQ_STORE, META_STORE, LOCAL_STORE], 'readwrite');
  if (txnResult.error) {
    return callback(txnResult.error);
  }
  var txn = txnResult.txn;
  ...
  txn.oncomplete = function () {
    ...
    var deleteReq = indexedDB.deleteDatabase(name);
    ...
    deleteReq.onerror = idbError(callback);
  };
}

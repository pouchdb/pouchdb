...
function init(api, opts, callback) {
  ...
  api._changes = function (opts) {
    ...
    function fetchChanges() {
      ...
      var range = descending ? null : IDBKeyRange.lowerBound(opts.since, true);

      if (!descending && !docIds && !opts.filter &&
          bySeqStore.getAll && bySeqStore.getAllKeys) {
        var idbLimit = limit === -1 ? null : limit;
        bySeqStore.getAllKeys(range, idbLimit).onsuccess = function (e) {
          var keys = e.target.result;
          bySeqStore.getAll(range, idbLimit).onsuccess = function (e) {
      ...
      if (descending) {
        req = bySeqStore.openCursor(range, descending);
      } else {
        req = bySeqStore.openCursor(range);
      }

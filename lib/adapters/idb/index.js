...
function init(api, opts, callback) {
  ...
  function addSecondaryIndexStore(db) {
    db.objectStore(DOC_STORE).createIndex('idx', 'idx', {
      unique: true,
      multiEntry: true
    });
  }
  ...
  api._createIndex = function (indexName, opts, callback) {
    api._indexers = api._indexers || {};
    api._indexers[indexName] = opts.indexer;
  ...
  req.onupgradeneeded = function (e) {
    ...
    if (e.oldVersion < 6) {
      addSecondaryIndexStore(db); // v5 -> v6
    }

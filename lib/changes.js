...
function Changes(db, opts, callback) {
  ...
  opts.onChange = function (change) {
    ...
    if(Array.isArray(change.seq))
    {
      //BigCouch returns array
      if (self.startSeq && self.startSeq <= change.seq[0]) {
        self.emit('uptodate');
        self.startSeq = false;
      }
    }
    else if (self.startSeq && self.startSeq <= change.seq) {
      self.emit('uptodate');
      self.startSeq = false;
    }
    if (change.deleted) {
  ...
  if (opts.continuous && opts.since !== 'now') {
    this.db.info().then(function (info) {
      if(Array.isArray(info.update_seq)){
        //BigCouch return array
        self.startSeq = info.update_seq[0] - 1;
      }
      else{

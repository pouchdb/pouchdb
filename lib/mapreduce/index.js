...
function readAttachmentsAsBlobOrBuffer(res, db) {
  // TODO: this will go away when mapreduce is back in pouch core
  var b64ToBluffer = db.constructor.utils.base64StringToBlobOrBuffer;
...
function postprocessAttachments(opts, db) {
  return function (res) {
    if (opts.include_docs && opts.attachments && opts.binary) {
      readAttachmentsAsBlobOrBuffer(res, db);
    }
    return res;
  };
}
...
function httpQuery(db, fun, opts) {
  ...
  if (typeof fun === 'string') {
    ...
    return db.request({
      ...
      body: body
    }).then(postprocessAttachments(opts, db));
  }

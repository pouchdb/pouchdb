...
//purge utils
exports.collectParentLeafRevs = function (rev_tree, rev, depth, result) {
  //returns all the revs that should be purged together with rev
  if (rev_tree === []) {
    return result;
  }
  if (rev_tree[2].length === 0) {
    if (rev_tree[0] === rev) {
      result.push(depth + "-" + rev);
    }
  } else if (rev_tree[2].length === 1) {
    exports.removeRev(rev_tree[2][0], rev, depth + 1, result);
    if (result[result.length - 1] === (depth + 1) + "-" + rev_tree[2][0][0]) {
      result.push(depth + "-" + rev_tree[0]);
    }
  } else {
    for (var i = 0; i < rev_tree[2].length; i++) {
      exports.removeRev(rev_tree[2][i], rev, depth + 1, result);
      if (result[result.length - 1] === (depth + 1) + "-" + rev_tree[2][i][0]) {
...
exports.updateRevTree = function (rev_tree, rev) {
  var trees = merge.rootToLeaf(rev_tree).filter(function (path) {
    return path.ids[path.ids.length - 1].id !== rev;
  }).map(function (path) {
    return merge.pathToTreeWithPos(path);
  });

  return trees.reduce(function (prev, current, i, arr) {
    return merge.doMerge(prev, current, true).tree;
  }, [trees.shift()]);
};
...
exports.purgeArray = function (docsArray, purged, callback, purgeOneRev,
  object) {
  console.log("purge a");
  ...
  object.compactDocument(oneDoc.id, 0, function (err, res) {
    if (err) {
      callback(err);
      return;
    }
    ...
    purgeDocWithRevs(oneDoc.id, oneDoc.revs, [], function (err, res) {
      ...
      exports.purgeArray(docsArray, purged, callback, purgeOneRev, object);
    }, purgeOneRev);
  });
};

exports.purge = function (docsList, callback, purgeOneRev, object) {
  var revsArray = [];
  ...
  exports.purgeArray(revsArray, {}, function (err, res) {
    if (!err) {

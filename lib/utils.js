...
exports.getRevPath = function getRevPath(rev, rev_tree)
{
  var paths = merge.rootToLeaf(rev_tree);
  var path = arrayFirst(paths, function (arr) {
    return arr.ids.map(function (x) { return x.id; })
      .indexOf(rev.split('-')[1]) !== -1;
  });
...
exports.findAncestorRevPos = function findAncestorRevPos(path,
  possibleAncestors) {
  path = path.ids
    .map(function (rev) {
      return rev.id;
  ...
  possibleAncestors = possibleAncestors.map(function (rev) {
    return rev.split('-')[1];
  });

  var revPositions = possibleAncestors.map(function (id) {
    return path.indexOf(id);
  });

  return Math.max.apply(null, revPositions);
};

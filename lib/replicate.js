...
function replicate(repId, src, target, opts, returnValue) {
  ...
  var _changes = [ ]; // The actual changes instances
  var result = {
    ok: true,
    start_time: new Date(),
    docs_read: 0,
  ...
  function completeReplication() {
    ...
    if( _changes.length > 0 ){
      _changes.forEach( function( _change ){
        if( _change && _change.cancel ){
          _change.cancel( );
        }
      } );
    }
  ...
  function getChanges() {
    if (
      ...
      var _change = src.changes(changesOpts);
      _change.on('change', onChange)
      .then(onChangesComplete)
      .catch(onChangesError);
      _changes.push( _change );
    }
  }

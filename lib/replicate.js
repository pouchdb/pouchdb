  ...
  function getDiffs() {
    ...
    currentBatch.changes.forEach(function (change) {
      diff[change.id] = change.changes.map(function (x) {
        if (change.doc._rev === x.rev &&
          !(change.doc._attachments &&
            Object.keys(change.doc._attachments).length)) {
          currentBatch.docs.push(change.doc);
        } else {
          return x.rev;
        }
      }).filter(function (x) {
    ...
    Object.keys(diff).forEach(function (key) {
      if (diff[key].length) {
        newDiffs[key] = diff[key];
      }
    });
    if (Object.keys(newDiffs).length === 0) {
      return Promise.resolve(afterDiffs({}));
    }
    function afterDiffs(diffs) {
      if (returnValue.cancelled) {
        completeReplication();
        throw new Error('cancelled');
      }
    ...
    return target.revsDiff(newDiffs).then(afterDiffs);
  }

...
ChangesBatch.prototype.next = function (callback) {
  var self = this;
  if (this.inProgress) {
    callback(new Error('changes in progress'));
    return;
  }
  this.inProgress = true;
  this.db.changes(this.opts).on('error', callback)
    .on('complete', function (resp) {
      if (resp.results.length) {
        self.opts.since = resp.last_seq;
        callback(null, {
          done: false,

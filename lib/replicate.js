...
function Replication(opts) {
  EE.call(this);
  this.USED_EVENTS = ['cancel', 'change', 'complete', 'error', 'outofdate',
                      'uptodate'];
  this.cancelled = false;
  // true if this was created while starting a new replication process,
  // false if it is just mirrors behaviour of Replication from another process.
    ...
    return toPouch(target).then(function (target) {
      if (opts.server) {
        ...
        return genReplicationId(src, target, opts).then(function (repId) {
          function cleanUp() {
            delete target.activeReplications[repId];
          }

          if (repId in target.activeReplications) {
            replicateRet.reallyRunning = false;
            var activeRet = target.activeReplications[repId];
            activeRet.USED_EVENTS.forEach(function (e) {
              activeRet.on(e, function () {
                var newArgs = arguments.slice();
                newArgs.unshift(e);
                replicateRet.emit.apply(this, newArgs);
              });
            });
          } else {
            target.activeReplications[repId] = replicateRet;
            replicateRet.on('complete', cleanUp).on('error', cleanUp);

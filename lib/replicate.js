    ...
    return target.bulkDocs({
      ...
      res.forEach(function (res) {
        ...
        if (res.error) {
          ...
          error_msg = ['reason', 'message'].filter(function (prop) {
            return typeof res[prop] === 'string';
          })[0];
          error_msg = error_msg && res[error_msg] || 'Unknown reason';
          error = new Error(error_msg);
          ['id', 'reason', 'status'].forEach(function (prop) {

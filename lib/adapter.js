...
function allDocsKeysQuery(api, opts, callback) {
  var keys = opts.keys;
  var skip = typeof opts.skip === 'number' ? opts.skip : 0;
  keys = ('limit' in opts) ? keys.slice(skip, opts.limit + skip) :
                  (opts.skip > 0) ? keys.slice(skip) : keys;
  if (opts.descending) {
    keys.reverse();
  }
  if (!keys.length) {
  ...
  Promise.all(keys.map(function (key, i) {
    ...
    ['limit', 'skip', 'keys', 'descending'].forEach(function (optKey) {

...
var debug = require('debug')('pouchdb:adapter');
...
AbstractPouchDB.prototype.get =
  ...
  return this._get(id, opts, function (err, result) {
    ...
    if (opts.revs || opts.revs_info) {
      path = utils.getRevPath(doc._rev, metadata.rev_tree);
      path.ids.reverse();
    ...
    if (opts.attachments && doc._attachments) {
      ...
      var attsSinceRevPos = -1;
      if (opts.atts_since) {
        path = utils.getRevPath(doc._rev, metadata.rev_tree);
        attsSinceRevPos = utils.findAncestorRevPos(path, opts.atts_since);
      }

      Object.keys(attachments).forEach(function (key) {
        this._getAttachment(attachments[key],
                            {encode: true, ctx: ctx}, function (err, res) {
          var revPos = res.revPos;
          var att = doc._attachments[key];

          debug('GET attachment \"%s\": revPos %s, attsSinceRevPos %s',
            key, revPos, attsSinceRevPos);

          // atts_since doesn’t includes attachments for specified revisions
          // always return (legacy) attachments with no revPos defined
          if (typeof revPos === "undefined" || revPos > attsSinceRevPos) {
            att.data = res.data;
            delete att.stub;
            delete att.length;
          }
...
AbstractPouchDB.prototype.getAttachment =
  ...
  this._get(docId, opts, function (err, res) {
    ...
    if (res.doc._attachments && res.doc._attachments[attachmentId]) {
      opts.ctx = res.ctx;
      self._getAttachment(res.doc._attachments[attachmentId], opts,
        function (err, res) {
          callback(err, res && res.data);
        });
    } else {

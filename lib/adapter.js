...
AbstractPouchDB.prototype.registerDependentDatabase =
  ...
  self.info(function (err, info) {
    ...
    // need to call before the constuctor so that all-dbs can catch it in time
    self.constructor.emit('registeredDependentDb', info.db_name, dependentDb,
      self.adapter);
    self.emit('registeredDependentDb', dependentDb);

    var depDB = new self.constructor(dependentDb, {adapter: self._adapter});
    ...
    upsert(self, '_local/_pouch_dependentDbs', diffFun, function (err) {
      if (err) {
        return callback(err);
      }

...
function indexersFromDesignDocs(db, ddocs) {
  var indexers = {};
  ddocs.forEach(function (ddoc) {
    if (!(ddoc.views && typeof ddoc.views === 'object')) {
      return;
    }
    Object.keys(ddoc.views).forEach(function (viewName) {
      var view = ddoc.views[viewName];
      var fullViewName = ddoc._id.substring(8) + '/' + viewName;
      db.constructor.secondaryIndexers.forEach(function (secondaryIndexer) {
        var indexer = secondaryIndexer(viewName, view);
        if (indexer) {
          indexers[fullViewName] = indexer;
        }
      });
    });
  });
...
function fetchIndexers(db, cb) {
  db.allDocs({
    ...
    var ddocs = res.rows.map(function (row) {
      return row.doc;
    });
    var indexers = indexersFromDesignDocs(db, ddocs);
    cb(null, indexers);
  });
}
...
AbstractPouchDB.prototype.bulkDocs =
  ...
  var self = this;
  fetchIndexers(self, function (err, indexers) {
    if (err) {
      return callback(err);
    }
    opts.indexers = indexers;
    self._bulkDocs(req, opts, function (err, res) {
      if (err) {
        return callback(err);
      }
      if (!opts.new_edits) {

...
function replicate(src, target, opts, returnValue, result) {
  ...
  function getOldDocs() {
    var docs = currentBatch.docs;
    if (currentBatch.docs.length === 0 || !opts.include_old_docs) {
      return;
    }

    // Get ids of documents to fetch
    // Don't fetch old versions of new documents
    var ids = docs.filter(function(doc) {
      return !/^1-/.test(doc._rev);
    }).map(function(doc) {
      return doc._id;
    });

    return target.allDocs({include_docs: true, keys: ids}).then(function (res) {
      if (returnValue.cancelled) {
        completeReplication();
        throw new Error('cancelled');
      }
  ...
  function finishBatch() {
    ...
    if (changedDocs.length) {
      if (opts.include_old_docs) {
        outResult.old_docs = oldDocs;
      }

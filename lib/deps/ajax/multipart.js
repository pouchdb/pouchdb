...
function createMultipart(req) {
  ...
  req.docs.forEach(function (doc) {
    ...
    var followAttachments = {};
    Object.keys(doc._attachments).forEach(function (filename) {
      var att = doc._attachments[filename];
      if (att.stub) {
        followAttachments[filename] = att;
        return;
      }
      var binData = atob(att.data);
      nonStubAttachments[filename] = {type: att.content_type, data: binData};
      followAttachments[filename] = {
        length: binData.length,
        follows: true
      };
    });

    // mutate, stringify, un-mutate
    var oldAttachments = doc._attachments;
    doc._attachments = followAttachments;
    parts.push(JSON.stringify(doc));

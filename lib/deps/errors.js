...
var generateErrorFromResponse = function (res) {
  var theError, errName, errType, errMsg, errReason;
  ...
  theError = error(errType, errReason, errName);

  // Keep custom message.
  if (errMsg) {
    theError.message = errMsg;
  }
  ...
  if (res.status) {
    theError.status = res.status;
  }
  if (res.missing) {
    theError.missing = res.missing;
  }

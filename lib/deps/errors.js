...
exports.generateErrorFromResponse = function (res) {
  ...
  errType = errors.getErrorTypeByProp('name', res.error, res.reason);
  ...
  } else if (res.error === 'doc_validation') {
    // doc validation needs special treatment since
    // res.reason depends on the validation error.
    // see utils.js
    errType = errors.DOC_VALIDATION;
    errMsg = res.reason;
  } else if (res.error === 'bad_request' && errType.message !== res.reason) {
    // if bad_request error already found based on reason don't override.
    errType = errors.BAD_REQUEST;
  }

  if (errType) {
    error = errors.error(errType, res.reason);
  } else {
    error = errors.error(errors.UNKNOWN_ERROR, res.reason, res.error);
  }

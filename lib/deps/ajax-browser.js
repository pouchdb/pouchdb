...
function ajax(options, adapterCallback) {
  ...
  function onError(err, cb) {
    ...
    try {
      ...
      if (err.status === 408) { // client-side timeout
        errObj = err;
      } else {
  ...
  var abortReq = function (err) {
    if (requestCompleted || aborted) {
      return;
    }
    aborted = true;
    xhr.abort();
    onError(err || xhr, callback);
  };
  var abortWithTimeout = function () {
    abortReq(errors.TIMEOUT);
  };

...
function createAbstractMapReduce(localDocName, mapper, reducer, ddocValidator) {
  ...
  async function httpQuery(db, fun, opts) {
    ...
    if (typeof opts.keys !== 'undefined') {
      const MAX_URL_LENGTH = 2000; // Maybe this is KiloByte?
      // In general, the web platform does not have limits on the length of URLs 
      // (although 2^31 is a common limit). Chromium uses 2MB so 2000000 is fine
  ...
  async function updateViewInQueue(view, opts) {
    ...
    function createTask() {
      return view.sourceDB.info().then(async (info) => {
        taskId = await view.sourceDB.activeTasks.add({
          name: 'view_indexing',

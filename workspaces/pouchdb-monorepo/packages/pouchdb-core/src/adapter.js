...
class AbstractPouchDB extends EventEmitter {
  _setup() {
    ...
    this.put = adapterFun('put', function (doc, opts, cb) {
      ...
      async function transformForceOptionToNewEditsOption() {
        var parts = doc._rev.split('-');
        var oldRevId = parts[1];
        var oldRevNum = parseInt(parts[0], 10);

        var newRevNum = oldRevNum + 1;
        var newRevId = await rev();
      ...
      if (opts.force && doc._rev) {
        transformForceOptionToNewEditsOption().then(()=>putDoc(function (err) {
    ...
    this.putAttachment = adapterFun('putAttachment', function (docId, attachmentId, rev, blob, type) {
      ...
      if (!type) {
        console.log('warn', 'Attachment', attachmentId, 'on document', docId, 'is missing content_type');
      }
        ...
        if (doc._attachments) {
          Object.keys(doc._attachments).forEach(function (name) {
            ...
            if (!doc._attachments[name].content_type) {
              console.log('warn', 'Attachment', name, 'on document', doc._id, 'is missing content_type');
            }
          });
        }
      });
  ...
  _compact(opts, callback) {
    ...
    this.info().then(async (info) => {
      taskId = await this.activeTasks.add({
